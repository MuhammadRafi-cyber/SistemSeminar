...
public class UserDAO {
    ...
    public int registrasi(User user) throws SQLException {
        String sql = "INSERT INTO user (id_institusi, nama, username, email, password_hash, role, no_telepon) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection c = Koneksi.getConnection();
            ...
            ps.setString(7, user.getNoTelepon());
            ps.executeUpdate();
    ...
    public boolean usernameSudahAda(String username) throws SQLException {
    ...
    public User cariByEmail(String email) throws SQLException {
        String sql = "SELECT id_user, id_institusi, nama, username, email, password_hash, "
                   + "role, no_telepon, tanggal_daftar FROM user WHERE email = ?";
        try (Connection c = Koneksi.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, email);
    ...
    public User cariById(int idUser) throws SQLException, DataTidakDitemukanException {
        String sql = "SELECT id_user, id_institusi, nama, username, email, password_hash, "
                   + "role, no_telepon, tanggal_daftar FROM user WHERE id_user = ?";
        try (Connection c = Koneksi.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, idUser);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public boolean updateProfil(int idUser, String nama, String username,
                                 String noTelepon, Integer idInstitusi) throws SQLException {
        String sql = "UPDATE user SET nama = ?, username = ?, no_telepon = ?, id_institusi = ? WHERE id_user = ?";
        try (Connection c = Koneksi.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, nama);
            ps.setString(2, username);
            ps.setString(3, noTelepon);
            if (idInstitusi != null) ps.setInt(4, idInstitusi); else ps.setNull(4, Types.INTEGER);
            ps.setInt(5, idUser);
    ...
    private User mapRow(ResultSet rs) throws SQLException {
        ...
        return switch (role) {
            case PESERTA -> new Peserta(idUser, idInstitusi, nama, username, email, passwordHash, noTelepon, tanggalDaftar);
            case PANITIA -> new Panitia(idUser, idInstitusi, nama, username, email, passwordHash, noTelepon, tanggalDaftar);
            case ADMIN   -> new Admin(idUser, idInstitusi, nama, username, email, passwordHash, noTelepon, tanggalDaftar);
        };
    }
}

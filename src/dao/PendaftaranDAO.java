...
public class PendaftaranDAO {
     ...
     * Rollback otomatis jika ada yang gagal.
     */
    public int insertDenganDetail(Pendaftaran p) throws SQLException {
        ...
        try {
            ...
            try (PreparedStatement ps = c.prepareStatement(sqlHeader, Statement.RETURN_GENERATED_KEYS)) {
                ps.setInt(1,    p.getIdPemesan());
                ps.setInt(2,    p.getIdSeminar());
                ps.setString(3, p.getKodeTransaksi());
                ps.setString(4, p.getStatus().name());
                ps.setDouble(5, p.getTotal());
                ps.executeUpdate();
                try (ResultSet rs = ps.getGeneratedKeys()) {
                    if (!rs.next()) throw new SQLException("Gagal mendapatkan ID pendaftaran baru.");
                    idPendaftaran = rs.getInt(1);
                    p.setIdPendaftaran(idPendaftaran);
                }
            }

            // 2. Insert tiket (detail_pendaftaran) — batch
            ...
            try (PreparedStatement ps = c.prepareStatement(sqlDetail, Statement.RETURN_GENERATED_KEYS)) {
                for (DetailPendaftaran d : p.getDetailList()) {
                    ...
                    ps.setString(2, d.getNamaPeserta());
                    ps.setString(3, d.getEmailPeserta());
                    ps.setString(4, d.getNoTelepon());
                    ps.setString(5, d.getKodeBooking());
                    ps.setString(6, d.getQrData());
                    ps.addBatch();
                }
                ps.executeBatch();
                try (ResultSet rs = ps.getGeneratedKeys()) {
    ...
    /** F1 / C5: riwayat pendaftaran seorang pemesan, join seminar */
    ...
    public List<DetailPendaftaran> getDetailByPendaftaran(int idPendaftaran) throws SQLException {
        String sql = "SELECT id_detail, id_pendaftaran, nama_peserta, email_peserta, "
                   + "no_telepon, kode_booking, qr_data "
                   + "FROM detail_pendaftaran WHERE id_pendaftaran = ?";
    ...
    public DetailPendaftaran getDetailByKodeBooking(String kode)
        ...
        String sql = "SELECT id_detail, id_pendaftaran, nama_peserta, email_peserta, "
                   + "no_telepon, kode_booking, qr_data "
                   + "FROM detail_pendaftaran WHERE kode_booking = ?";
        try (Connection c = Koneksi.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, kode);
            try (ResultSet rs = ps.executeQuery()) {

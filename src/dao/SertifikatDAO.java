...
public class SertifikatDAO {
    ...
    public int insert(int idDetail, String nomorSertifikat, String filePath) throws SQLException {
        String sql = "INSERT INTO sertifikat (id_detail, nomor_sertifikat, versi, file_path) "
                   + "VALUES (?, ?, 1, ?)";
        try (Connection c = Koneksi.getConnection();
             PreparedStatement ps = c.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setInt(1, idDetail);
            ps.setString(2, nomorSertifikat);
    ...
    /** Regenerate: update nomor_sertifikat, naikkan versi, update file_path. */
    public boolean regenerate(int idDetail, String nomorBaru, String filePathBaru) throws SQLException {
        String sql = "UPDATE sertifikat SET nomor_sertifikat = ?, versi = versi + 1, "
                   + "file_path = ?, tanggal_terbit = NOW() WHERE id_detail = ?";
        try (Connection c = Koneksi.getConnection();
    ...
    public Sertifikat getByDetail(int idDetail) throws SQLException {
        String sql = "SELECT id_sertifikat, id_detail, nomor_sertifikat, tanggal_terbit, versi, file_path "
    ...
    /** E2: sertifikat milik seorang pemesan. */
    public List<Object[]> getSertifikatByUser(int idUser) throws SQLException {
        String sql =
            "SELECT st.nomor_sertifikat, st.tanggal_terbit, st.versi, st.file_path, "
    ...
    private Sertifikat mapRow(ResultSet rs) throws SQLException {
        return new Sertifikat(
            rs.getInt("id_sertifikat"), rs.getInt("id_detail"),
            rs.getString("nomor_sertifikat"), rs.getString("tanggal_terbit"),
            rs.getInt("versi"), rs.getString("file_path")
        );
    }
}
